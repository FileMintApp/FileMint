...
修复应用内更新安装失败：正式包签名时会正确写入安装器权限，并检查签名中的实际权限。已确认 0.5.7/0.5.8 受影响，请手动安装 0.5.9 一次。

[0.5.9 发布说明](https://github.com/FileMintApp/FileMint/releases/tag/v0.5.9)

## 近期功能

- **使用 App 打开**：把常用应用加入 Finder 菜单，打开所选文件或文件夹。
- **图片粘贴为文件**：预览并命名拷贝的截图或图片，再保存为 PNG。
- **同格式多模板**：为同一种格式保存不同内容和默认文件名，并指定默认模板。
...
- **文件创建**：模板与类型、创建行为。
- **扩展功能**：文件（夹）工具、资源工具、使用 App 打开。
- **偏好设置**：通用、Finder 与文件夹、关于。

每个页面有明确的职责，扩展模块可以单独关闭，方向键可在页面间移动。在「通用 → 外观」选择跟随系统、浅色或深色，默认跟随系统。
...
**0.5.9 安装包使用 Developer ID 签名，通过 Apple 公证并附加公证票据。** 首次使用仍需按系统提示启用 Finder 扩展和文件夹授权。
...
安装 0.5.9 后，可通过 **关于 → 检查更新 → 更新并重启** 获取后续版本。请先完成创建或编辑操作；macOS 可能要求管理员授权。

旧版本请手动安装一次 0.5.9：退出 FileMint，将新版拖入“应用程序”替换，推出安装磁盘后重新打开。0.5.7/0.5.8 的签名权限问题无法通过旧版自身的更新器修正。
