...
在已授权范围内选中一个或多个文件或文件夹后，右键会出现与“新建文件”并列的“文件（夹）工具”菜单。
模块默认关闭，不会打扰原有的创建菜单。

- **先按需开启。** 打开 FileMint → 设置 → 扩展功能 → 文件（夹）工具。开启总开关后，可分别选择“拷贝文件（夹）名称”“拷贝文件（夹）路径”和“移动文件（夹）”。
- **拷贝名称或路径。** 名称保留后缀；多选时每项各占一行。路径是完整的本地文件系统路径。只有点按这两个菜单项才会写入剪贴板，不会读取文件内容。
- **移动分两步完成。** 先在源项目上选择“移动文件（夹）”，再到目标文件夹的空白处右键，选择最外层的“将所选项目移到此处”。同名项目绝不覆盖或合并；再次选择源项目会替换上一批待移动项目，未完成项目可在解决问题后重新尝试。
...
- **选中的项目，按需处理。** 文件（夹）工具把名称、路径拷贝和两步移动放在独立 Finder 菜单里；默认关闭，也不改变“新建文件”的行为。
...
**文件（夹）工具：** 先在“设置 → 扩展功能 → 文件（夹）工具”开启总开关。在已授权范围内选中项目后右键，
选择“文件（夹）工具”即可拷贝名称、拷贝路径或准备移动；移动需在目标文件夹空白处再次右键确认。
