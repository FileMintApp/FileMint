...
**0.5.5 安装包使用 Developer ID 签名，通过 Apple 公证并附加公证票据。** Finder 扩展启用与文件夹授权仍需在首次使用时完成。
...
0.5.5 及之后包含新更新器的版本：**关于 → 检查更新 → 更新并重启**，下载并校验后自动替换和重启。
请先完成创建或编辑操作；安装位置权限不足时，macOS 可能要求管理员授权。
使用旧更新器的版本（包括 0.5.4）仍需手动安装一次 0.5.5：
