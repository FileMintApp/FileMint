...
包含新更新器的版本：**关于 → 检查更新 → 更新并重启**，下载并校验后自动替换和重启。
请先完成创建或编辑操作；安装位置权限不足时，macOS 可能要求管理员授权。
目前使用旧更新器的版本（包括 0.5.4）仍需手动安装一次包含新更新器的版本：
...
不上传文件名、路径、内容；不收集使用数据；只在你主动粘贴时读取剪贴板。默认低频自动检查更新，也可手动检查或下载，这些操作会连接 GitHub；自动检查可在“通用”关闭。无需注册，无需订阅。见[隐私说明](PRIVACY.md)。
