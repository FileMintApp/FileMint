...
[[ "$BUILD_NUMBER" =~ ^[1-9][0-9]*$ ]] || { echo "Invalid release build"; exit 2; }
if [[ "${NOTARIZE:-0}" == 1 && ( "$VERSION" != "$SOURCE_VERSION" || "$BUILD_NUMBER" != "$SOURCE_BUILD" ) ]]; then
  echo 'Public release version/build must match project.yml' >&2
  exit 2
fi
mkdir -p "$OUTPUT_DIR"
