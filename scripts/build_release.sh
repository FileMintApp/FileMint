...
if [[ -n "${MARKETING_VERSION:-}" ]]; then XCODEBUILD_ARGS+=(MARKETING_VERSION="$MARKETING_VERSION"); fi
if [[ -n "${CURRENT_PROJECT_VERSION:-}" ]]; then XCODEBUILD_ARGS+=(CURRENT_PROJECT_VERSION="$CURRENT_PROJECT_VERSION"); fi
